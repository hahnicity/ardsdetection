...
base_path = os.path.join(os.path.abspath(args.dataset_path), 'experiment1/all_data/raw/')
if not os.path.exists(base_path):
    raise Exception('You must convert the old "training" dir to all_data. You must do this manually')
new_training_path = os.path.join(os.path.abspath(args.dataset_path), 'experiment1/training/raw/')
if os.path.exists(new_training_path):
    raise Exception('You must convert the old "training" dir to all_data. You must do this manually')
new_testing_path = os.path.join(os.path.abspath(args.dataset_path), 'experiment1/testing/raw/')
...
for pt in train_ards + train_other:
    proc = subprocess.Popen(['ln', '-s', base_path + pt, new_training_path])
    proc.communicate()

for pt in test_ards + test_other:
    proc = subprocess.Popen(['ln', '-s', base_path + pt, new_testing_path])
    proc.communicate()
